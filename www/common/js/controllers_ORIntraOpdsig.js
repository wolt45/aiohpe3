IOHPEApp.controller('ORPinIntraOp_Ctrl', function ($scope, $routeParams, $http){
  $scope.jdata_ORIntraOpdsig = [];
  $scope.ClinixRID = $routeParams.p_clinixrid;


   $scope.IntraPinchecking = function(formPin) 
  {
    ...
    }else
    {
      ...
      promise.then(function(pxresult) {
        ...
        if(angular.isDefined(pxresult[0]['PIN']))
        {
          ...
          tx.executeSql("delete from 'jdata_ORIntraOpdsig' WHERE ClinixRID = " + $scope.clinix.ClinixRID + " AND PxRID = " +$scope.clinix.PxRID);
          });
        }
        ...
        $ipadrbg.context.jdata_ORIntraOpdsig.add(newrecord);
        $ipadrbg.context.jdata_ORIntraOpdsig.saveChanges();
