IOHPEApp.controller('PreOpKneedsig_Ctrl', function ($scope, $routeParams, $http){
  $scope.jdata_PreOpKneedsig = [];
  $scope.ClinixRID = $routeParams.p_clinixrid;


   $scope.PreOpKneePinchecking = function(formPin) 
  {
      ...
      promise.then(function(pxresult) {
        ...
        $scope.jdata_PreOpKneedsig = pxresult;
          if(angular.isDefined(pxresult[0]['PIN']))
          {
            ...
            tx.executeSql("delete from 'jdata_PreOpKneedsig' WHERE ClinixRID = " + $scope.clinix.ClinixRID + " AND PxRID = " +$scope.clinix.PxRID);
            });
          }
          ...
          $ipadrbg.context.jdata_PreOpKneedsig.add(newrecord);
          $ipadrbg.context.jdata_PreOpKneedsig.saveChanges();
