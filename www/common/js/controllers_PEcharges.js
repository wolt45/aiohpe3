...
IOHPEApp.controller('PEchargesCtrl', function ($scope, $routeParams, $http){
  ...
  $scope.ClosePE = function (clinix) {
    if (confirm('Are you sure to Close this Appoinment: ' + $scope.ClinixRID + ' ?')) {
      ...
      clinix.TranStatusDisp = "UNPAID"; // UNPAID
      ...
      // Push Diagnosis, this Clinix.PE onlye   1.99
  ...
  $scope.notifyServer = function () {
    ...
    $http({
      method: 'POST'
      , url : 'http://192.168.1.99/RBGsrvr_todayset/srvr_back_clinixTrnStts.php?clinixJsonIzed=' + $scope.clinix_JSON
      , contentType : 'application/json'
    ...
    $http({
      method: 'POST'
      , url : 'http://192.168.1.99/RBGsrvr_todayset/srvr_back_PEcharges.php?clinixJsonIzed=' + $scope.clinix_PEcharges_JSON
      , contentType : 'application/json'
