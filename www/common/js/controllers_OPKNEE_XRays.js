    ...
    promise.then(function(pxresult) {
      $scope.$apply(function () {
        ...
        for(var i = 0; i <= $scope.AllPreOpKNEEVid.length; i++) {
          var vidurl = $sce.trustAsResourceUrl("http://10.0.1.99/dump_labs/" + $scope.AllPreOpKNEEVid[i]['ImageFileName']);
    ...
    promise.then(function(pxresult) {
      $scope.$apply(function () {
        ...
        for(var i = 0; i <= $scope.AllPostOpKNEEVid.length; i++) {
          var vidurl = $sce.trustAsResourceUrl("http://10.0.1.99/dump_labs/" + $scope.AllPostOpKNEEVid[i]['ImageFileName']);
